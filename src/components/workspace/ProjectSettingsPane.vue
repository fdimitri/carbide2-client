...
<template>
        ...
        <section class="mb-7">
          ...
          <div class="mt-3">
            <label class="flex items-center gap-2 text-[0.82rem] text-text cursor-pointer select-none w-fit" @click.prevent="onWipeCheckboxClick">
              ...
              <span
                class="inline-flex items-center justify-center w-4 h-4 rounded-full border border-[#587296] text-[0.65rem] text-muted cursor-pointer hover:border-accent hover:text-accent transition-colors leading-none"
  ...
  <Dialog v-model:visible="showWipeConfirm" modal :closable="false" :style="{ width: '32rem' }">
    <template #header>
      <span class="text-[#ffb9c8] font-bold text-base tracking-tight">⚠ Are you absolutely sure?</span>
    </template>

    <p class="text-text text-sm mb-2">
      This change <strong class="text-[#ffb9c8]">CANNOT BE REVERTED</strong>. You will
      <strong class="text-[#ffb9c8]">PERMANENTLY LOSE ALL FILE HISTORY</strong> stored in the
    ...
    <label class="flex items-start gap-2 cursor-pointer select-none text-sm text-text">
      <input type="checkbox" v-model="wipeConfirmed" class="accent-[#ff6b6b] mt-[0.15rem] shrink-0" />
    ...
    <template #footer>
      <div class="flex items-center gap-3 w-full">
        ...
        <button
          ...
          class="px-4 py-2 rounded-lg bg-[#4d1b27] border border-[#ff6b6b] text-[#ffb9c8] text-sm font-bold cursor-pointer
      ...
      h('button', {
        ...
        class: [
          ...
          props.activeHelp === props.helpKey
            ...
            : 'border-[#587296] text-muted hover:border-accent hover:text-accent',
        ],
